...
pushd "$DIR/Build/$ARCH"
    ...
    pushd gcc
        ...
        echo "XXX TheOS libc and libm headers"
        mkdir -p "$BUILD"
        pushd "$BUILD"
            mkdir -p Root/usr/include/
            SRC_ROOT=$($REALPATH "$DIR"/..)
            FILES=$(find "$SRC_ROOT"/Userland/Libs/LibC "$SRC_ROOT"/Userland/Libs/LibM -name '*.h' -print)
            for header in $FILES; do
                target=$(echo "$header" | sed -e "s@$SRC_ROOT/Userland/Libs/LibC@@" -e "s@$SRC_ROOT/Userland/Libs/LibM@@")
