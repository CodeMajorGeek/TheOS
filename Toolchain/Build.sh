...
pushd "$DIR/Build/$ARCH"
    ...
    echo "XXX theos libc headers"
    mkdir -p "$BUILD"
    pushd "$BUILD"
        mkdir -p Root/usr/include/
        SRC_ROOT=$($REALPATH "$DIR"/..)
        FILES=$(find "$SRC_ROOT"/Userland/Libraries/LibC -name '*.h' -print)
        for header in $FILES; do
            target=$(echo "$header" | sed -e "s@$SRC_ROOT/Userland/Libraries/LibC@@")
    ...
    if [ ! -f "$DIR/Tarballs/gcc-$GCC_VERSION/gcc/config/theos-userland.h" ]; then
        cp "$DIR/Tarballs/gcc-$GCC_VERSION/gcc/config/theos.h" "$DIR/Tarballs/gcc-$GCC_VERSION/gcc/config/theos-kernel.h"
    fi
    ...
    pushd gcc
        ...
        buildstep "gcc+libgcc/install" "$MAKE" -j "$MAKEJOBS" DESTDIR="$TEMPTARGET" install-gcc install-target-libgcc || exit 1

        cp -a "$TEMPTARGET$PREFIX"/* $PREFIX
        rm -rf "$TEMPTARGET"
    popd
popd
